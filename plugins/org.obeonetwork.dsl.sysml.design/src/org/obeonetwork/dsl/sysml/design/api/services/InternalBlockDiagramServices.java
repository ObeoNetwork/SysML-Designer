...
public class InternalBlockDiagramServices extends SysmlAbstractDiagramServices {
	...
	public Port createProxyPort(final EObject container) {
		...
		port.setName(computeSysmlLabel(port));
		SysmlElementServices.INSTANCE.createAssociatedStereotype(port, "SysML::PortsAndFlows", "ProxyPort"); //$NON-NLS-1$//$NON-NLS-2$
		return port;
	}
	...
	/**
	 * Get the nested ports.
	 *
	 * @param container
	 *            The container
	 * @return List of the nested ports
	 */
	public List<Port> getNestedPorts(Port port) {
		final List<Port> results = Lists.newArrayList();
